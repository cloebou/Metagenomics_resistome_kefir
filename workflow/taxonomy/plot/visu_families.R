...
# group rare taxa
	df <- df %>%
		...
		ungroup() %>%
		mutate(name = ifelse(mean_p < threshold,paste0("Other <", threshold, "%"),
							 name)) %>%
		group_by(sample, name) %>%
		summarise(percent = sum(percent), .groups = "drop")
	ggplot(df, aes(x = sample, y = percent,fill = name)) +
...
for (i in seq_along(sample_groups)) {
	...
	make_stacked(
		...
		outfile = file.path(out_dir, paste0("families_stacked_", idx, ".png")),
		title = paste("Families-level composition (group", idx, ")")
	)
	c=c+1
	}
